...
import { BriefcaseBusiness, ChartNoAxesCombined, CircleEllipsis, MessageCircle, ShoppingBag, Trophy } from 'lucide-vue-next'
import { siBaidu, siBilibili, siDouban, siGithub, siJuejin, siSinaweibo, siSteam, siTiktok, siZhihu } from 'simple-icons'
...
const brandIcons: Partial<Record<HotSource, { path: string; hex: string }>> = {
  ...
  BILIBILI: siBilibili,
  JUEJIN: siJuejin,
  DOUBAN_MOVIE: siDouban,
  GITHUB_TRENDING: siGithub,
  STEAM: siSteam,
}
...
const iconColor = computed(() => {
  ...
  if (props.source === 'NOWCODER') return '#00a98f'
  if (props.source === 'TIEBA') return '#4e6ef2'
  if (props.source === 'HUPU') return '#c51f25'
  if (props.source === 'DOUBAN_MOVIE') return '#2e963d'
  if (props.source === 'GITHUB_TRENDING') return '#24292f'
  if (props.source === 'STEAM') return '#1b2838'
  if (props.source === 'SMZDM') return '#e62828'
  if (props.source === 'DONGQIUDI') return '#16a05d'
  return '#7c858b'
})
</script>
...
<template>
  ...
  <BriefcaseBusiness v-else-if="source === 'NOWCODER'" class="platform-icon" :style="{ '--platform-icon-color': iconColor }" :size="16" :stroke-width="2.2" aria-hidden="true" />
  <MessageCircle v-else-if="source === 'TIEBA'" class="platform-icon" :style="{ '--platform-icon-color': iconColor }" :size="16" :stroke-width="2.2" aria-hidden="true" />
  <Trophy v-else-if="source === 'HUPU' || source === 'DONGQIUDI'" class="platform-icon" :style="{ '--platform-icon-color': iconColor }" :size="16" :stroke-width="2.2" aria-hidden="true" />
  <ShoppingBag v-else-if="source === 'SMZDM'" class="platform-icon" :style="{ '--platform-icon-color': iconColor }" :size="16" :stroke-width="2.2" aria-hidden="true" />
  <CircleEllipsis
