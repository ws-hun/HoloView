...
当前版本已经接入百度公开热榜真实数据，并具备完整的前后端分层、数据库模型、榜单查询、趋势曲线、SSE 实时更新和响应式前端。开发与测试环境仍保留 Mock Collector，新增平台只需实现统一采集接口，不需要重写页面。
...
- 真实数据：低频采集百度公开热榜，映射标题、摘要、排名、热度、封面与原始链接。
...
│   │   ├── service/                 # 查询服务
│   │   └── collector/               # 百度真实采集器、分类器与 Mock 采集器
│   └── src/main/resources/
